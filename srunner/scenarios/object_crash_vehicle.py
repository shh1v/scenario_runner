...
class DynamicObjectCrossing_proposal_hardcode(BasicScenario):
    ...
    def _spawn_adversary(self, transform, orientation_yaw, waypoint):
        ...
        adversary_location = transform.location
        adversary_location += offset_location * (0.72)
        self.transform = carla.Transform(adversary_location,
                                          transform.rotation)
        ...
        if self._adversary_type is False:
            ...
            # walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0004', transform)
            # walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0005', transform)
            walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0038', transform)
            # walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0042', transform)
            adversary = walker
        else:
    ...
    def _spawn_blocker(self, transform, orientation_yaw, waypoint):
        ...
        blocker_vehicle = 'vehicle.carlamotors.carlacola'
    ...
    def _initialize_actors(self, config):
        ...
        while True:  # We keep trying to spawn avoiding props
            try:
                ...
                first_vehicle = self._spawn_adversary(self.transform, orientation_yaw, waypoint)
    ...
    def _create_behavior(self):
        ...
        scenario_sequence.add_child(Idle(2))
...
# this class was created as a backup because the above class was modified for 
# Abhijat's thesis proposal example scenario.
class DynamicObjectCrossing(BasicScenario):
    ...
    def _spawn_adversary(self, transform, orientation_yaw, waypoint):
        ...
        offset_location = carla.Location(
            ...
            lane_width * math.sin(math.radians(position_yaw)))
        adversary_location = transform.location
        # adversary_location += offset_location
        self.transform = carla.Transform(adversary_location,
                                          transform.rotation)
        transform = self.transform
        ...
        if self._adversary_type is False:
            ...
            self._other_actor_target_velocity = 6 + (0.4 * self._num_lane_changes)
            # walker = CarlaDataProvider.request_new_actor('walker.*', transform)
            # walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0032', transform)
            walker = CarlaDataProvider.request_new_actor('walker.pedestrian.0021', transform)
            adversary = walker
        else:
            self._other_actor_target_velocity = self._other_actor_target_velocity * self._num_lane_changes
            first_vehicle = CarlaDataProvider.request_new_actor('vehicle.diamondback.century', transform)
