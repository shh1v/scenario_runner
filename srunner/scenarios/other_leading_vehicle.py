...
class OtherLeadingVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._first_vehicle_location = 15
        self._second_vehicle_location = self._first_vehicle_location - 1
        self._ego_vehicle_drive_distance = 500
    ...
    def _initialize_actors(self, config):
        ...
        """
        self._scenario_traffic_light = CarlaDataProvider.get_next_traffic_light_by_location(
            config.trigger_points[0].location)

        first_vehicle_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint,
                                                             self._first_vehicle_location)
        second_vehicle_waypoint, _ = get_waypoint_in_distance(self._reference_waypoint,
                                                              self._second_vehicle_location)

        # second vehicle is always spawned in left lane?
        second_vehicle_waypoint_temp = second_vehicle_waypoint.get_left_lane()
        if (second_vehicle_waypoint_temp is None or
            second_vehicle_waypoint_temp.lane_type != carla.LaneType.Driving):
            second_vehicle_waypoint_temp = second_vehicle_waypoint.get_right_lane()
        second_vehicle_waypoint = second_vehicle_waypoint_temp
    ...
    def _create_behavior(self):
        ...
        keep_velocity.add_child(InTriggerDistanceToVehicle(self.other_actors[0], self.ego_vehicles[0],
                                                           self._first_vehicle_speed))

        # Decelerating actor sequence behavior
        decelerated_speed = self._first_vehicle_speed / 6
        leading_actor_sequence_behavior.add_child(keep_velocity)
        slow_actor_autoparams = {
            "auto_lane_change": False,
            "max_speed": (self._first_vehicle_speed*100 /
                          self.other_actors[0].get_speed_limit()),  # this is a percentage
        }
        fast_actor_autoparams = {
            "auto_lane_change": True,
            "force_lane_change": False,  # change to left
            "max_speed": 100,  # this is a percentage
        }
        # leading_actor_sequence_behavior.add_child(ChangeAutoPilot(self.other_actors[0], True,
        #                                                           parameters=slow_actor_autoparams,
        #                                                           parameters=slow_actor_autoparams,
        #                                                           name="Slow actor auto"))
        leading_actor_sequence_behavior.add_child(WaypointFollower(self.other_actors[0], decelerated_speed,
                                                                   avoid_collision=True))
        # end condition
        end_condition = DriveDistance(self.other_actors[0], 250)

        # Build behavior tree
        sequence = py_trees.composites.Sequence("Other Leading Vehicle Scenario behavior")
        parallel_root = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        # TODO AutoPilot + this probably works if we do success on all instead of one
        # then we can remove the traffic light hack below!

        parallel_root.add_child(driving_in_same_direction)
        driving_in_same_direction.add_child(leading_actor_sequence_behavior)
        fast_vehicle_autopilot = ChangeAutoPilot(self.other_actors[1], True,
                        parameters=fast_actor_autoparams,
                        name="Fast actor autopilot on")
        # driving_in_same_direction.add_child(fast_vehicle_autopilot)
        ...
        start_condition = InTriggerDistanceToVehicle(
            ...
            self.ego_vehicles[0],
            20,
            name="Waiting for ego to get close to other_vehicle")

        scenario_light_green = TrafficJunctionSetterWithTime(
            actor=self._scenario_traffic_light,
            state=carla.TrafficLightState.Green,
            duration=10,
            name="Change scenario light to green")

        scenario_start = py_trees.composites.Sequence("Start the other actors' behavior")
        scenario_start.add_child(start_condition)
        scenario_start.add_child(scenario_light_green)
        ...
        sequence.add_child(scenario_start)
...
class OtherLeadingVehicle_backup(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(OtherLeadingVehicle_backup, self).__init__("VehicleDeceleratingInMultiLaneSetUp",
                                                  ego_vehicles,
                                                  config,
                                                  world,
                                                  debug_mode,
    ...
    def _create_behavior(self):
        ...
        # Decelerating actor sequence behavior
        decelerated_speed = self._first_vehicle_speed / 5
        leading_actor_sequence_behavior.add_child(keep_velocity)
        leading_actor_sequence_behavior.add_child(WaypointFollower(self.other_actors[0], decelerated_speed,
                                                                   avoid_collision=True))
        # end condition
