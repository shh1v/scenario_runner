...
class SpawnNPCsAtTransform(AtomicBehavior):

    """
    Implementation for a behavior that will create actors
    at a given transform.
    WARNING: This behaviour assumes that the spawn point in unoccupied
    ...
    def __init__(self, config_file_name, actor_index, name="SpawnVehicleAtTransform"):
        """
        Setup class members
        """
        super(SpawnNPCsAtTransform, self).__init__(name)
        self._world = CarlaDataProvider.get_world()
        self._map = self._world.get_map()
        self._actor_index = actor_index # Informs what actor to be spawned
        # WARNING: Hard coded Town04 setting in the config file.
        self._config = SpawnNPCsAtTransform._load_config(config_file_name).available_scenarios[0].Town04[0].available_event_configurations[0]
    
    @staticmethod
    def _load_config(filename):
        # Construct the full path by appending the directory path to the filename
        full_path = os.path.join('/srunner/data', f'{filename}.json')
    ...
    def update(self):
        try:
            # Figure out the transform of the vehicle
            ego_vehicle_location = carla.Location(self._config.ego_spawn_point.x, self._config.ego_spawn_point.y, self._config.ego_spawn_point.z)
            
            for actor in self._config.npc_actors:
                vehicle_waypoint, _ = get_waypoint_in_distance(self._map.get_waypoint(ego_vehicle_location),
                                                            actor.vehicle_offset, False, actor.lane)
                ...
                spawned_actor = CarlaDataProvider.request_new_actor(
                    model=self.actor_config.model, spawn_point=vehicle_transform, rolename=json.dumps(rolename_dict))
                
                # Add the spawned npc vehicle to the list of spawned actors
                SpawnNPCsAtTransform.spawned_npc_actors.append(spawned_actor)
                
        except:
            print("SpawnVehicleAtTransform unable to spawn actor")
        return py_trees.common.Status.SUCCESS
...
class SpawnEgoAdjacentVehicle(AtomicBehavior):
    def __init__(self, name="SpawnEgoAdjacentVehicle"):
        super(SpawnEgoAdjacentVehicle, self).__init__(name)
        self.logger.debug("%s.__init__()" % self.__class__.__name__)
        
        # Find the location of the ego vehicle

    def update(self):
        """
        Spawn the vehicles, and turn on autopilot
        """
